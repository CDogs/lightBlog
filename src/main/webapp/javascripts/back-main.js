...
function listenArticleEve() {
	...
	$("#btnSb,.btn-adele").click(function() {
		...
		if (action !== "" && action === "update") {
			reqUrl = "/user/article/update.htm";
		}
		//删除操作
	...
	function doAjax(_param) {
		//alert(contextPath);
		var type = "json";
		if(reqUrl == "/user/article/update.htm")type="text";
		$.ajax({
			...
			success : function (res) {
				...
				if (res == "SUCCESS") {
					...
					//window.location.replace(contextPath+"/user/ucenter.htm");
				} else if(res == "FAIL"){
					alert("操作失败！");
				}
			}
	...
	$(".btn-aupd").click(function() {
		...
		window.location.href = contextPath + "/user/article/init-edit.htm?id="+aId;
	});
	...
	//异步添加后展示文章
	function showAirticle(data){
		var tagsStr = "";
		$.each(data.tags,function(i,tag){
				var str = '<span class="label label-success" style="font-size: small">'+tag.name+'</span>&nbsp;&nbsp;';
			tagsStr = tagsStr + str;
		});
